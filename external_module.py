...
consistency_result_for_gpt = [] # con이라고 생각하면 True, 아니면 False
consistency_result_for_energy = [] # con이라고 생각하면 True, 아니면 False
...
for idx, filename in tqdm(enumerate(files)):
    ...
    ##### prediction을 고려하여 gpt에 넣기
    consistency_check_for_llm = model.wiqa_consistency_check_for_pred(one_json_file, predictions)
    # type(consistency_check_for_llm) == bool
    consistency_result_for_gpt.append(consistency_check_for_llm)


    ##### prediction을 고려하여 energy net에 넣기
    energy_input = "<s>"

    qa_pairs = one_json_file["qa_pairs"]
    for idx, qa_pair in enumerate(qa_pairs):
        energy_input += f" {qa_pair['question'][:-1]}? The answer is {predictions[idx]}.\n"
    ...
    total_result['energy_consistency_check_accuracy'].append(consistency_check_for_energy)
...
print(f"consistent rate = {sum(separate_inference_consistency_list)} / {len(separate_inference_consistency_list)} = {sum(separate_inference_consistency_list)/len(separate_inference_consistency_list):.4f}")
...
for key, val in infodict.items():
    print(f"key: {key}, val: {val} ({val/len(separate_inference_consistency_list):.4f})")
