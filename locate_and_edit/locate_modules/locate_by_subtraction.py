...
class locate_by_subtraction:
    ...
    def locate(self, pair, with_gold=True, threshold=None):
        ...
        if self.recursive_locate == False:
            locate_result = self.locate_only_once(
                [p[0] for p in pair], with_gold, threshold
            )
        else:
        ...
        locate_result["errors_gold"] = error_gold  # treat as a batch of size 1
    ...
    def locate_recursively(self, long_text, threshold=None, previous_returns=[]):
        ...
        for s_i in subtracted_inputs:
            ...
            if self.energynet.output_form == "2dim_vec":
                e_out, _ = self.energynet.energy_model(
                    [s_i], pair_only=True
                )  # (len(set), 2)
        ...
        #     return previous_returns + [prediction_string]
            ...
            return self.locate_recursively(
                new_pair, threshold, previous_returns + [prediction_string]
            )
