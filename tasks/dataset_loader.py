...
def dataset_loader(params, split = 'train', consistency = 'con', additional_info = {'pairwise': 'arbitrary_pairs', 'load_full': False}):
    if params['task'] == 'vqa':
        if params['dataset'] == 'lconvqa':
            dataset = l_convqa_dataset(params, split, consistency, additional_info['pairwise'])
        ...
        if params['dataset'] == 'set_nli':
            dataset = set_nli_dataset(params, split, consistency, additional_info['pairwise'], additional_info['load_full'])

    return dataset

def dataset_loader_fine_grained(params, split = 'train', semantic = 'con', pairwise = "two_pairs", load_in_once_everything = False, output_form ='real_num', input_type = "set"):
    if params['finetune_job_id'] == 0:
        if params['task'] == "mixed":
            seq_of_dataset = []
            seq_of_data_name = []
            if 'lconvqa' in params['dataset']:
                seq_of_dataset.append(l_convqa_fine_grained_dataset(params, split, semantic, pairwise, load_in_once_everything, output_form, input_type))
                seq_of_data_name.append('lconvqa')
            if 'set_nli' in params['dataset']:
                seq_of_dataset.append(set_nli_dataset(params, split, semantic, pairwise, load_in_once_everything, output_form, input_type))
            ...
            if params['dataset'] == 'lconvqa':
                dataset = l_convqa_fine_grained_dataset(params, split, semantic, pairwise, load_in_once_everything, output_form, input_type)
            ...
            if params['dataset'] == 'set_nli':
                dataset = set_nli_dataset(params, split, semantic, pairwise, load_in_once_everything, output_form, input_type)
...
def transform_arbitrary_pairs_to_diverse_pairs(seq_of_dataset):
    ...
    이 함수에서는, arbitrary con_dataset과 arbitrary incon_dataset만 대입함.
    이를 통해
...
def transform_arbitrary_pairs_to_two_pairs(seq_of_dataset):
    ...
    for dataset in seq_of_dataset:
        for data in dataset:
            if len(data) == 2:
                ...
                combinations = list(itertools.combinations(data, 2)) # "len(d)-choose-2"-number of combinations
    ...
    transformed_dataset_con = transformed_dataset_con[:min([len(s) for s in seq_of_dataset])]
    transformed_dataset_incon = transformed_dataset_incon[:min([len(s) for s in seq_of_dataset])]

    return [general_dataset(transformed_dataset_con), general_dataset(transformed_dataset_incon)], ['con_two_pairs', 'incon_two_pairs'], set_sizes

def concat_arbitrary_pairs(seq_of_dataset, concat_num = 2):
    """
    seq_of_dataset = [con_dataset, incon_dataset] 딱 두 개
    """
    ...
    semantic_combinations = [(concat_num-i, i) for i in range(concat_num+1)] # con과 incon을 몇대몇 비율로 섞을 것인지에 대한 모든 경우의 수 나열
    for s_comb in semantic_combinations:
        one_dataset = []
        random.Random(1014).shuffle(index_combinations)
        idx_combs = index_combinations[:data_num] # e.g., concat_num=3일 때, idx_combs = [(1,2,3), (3,6,2), ...]
        ...
        for indices in idx_combs:
            tmp = [] # one data. 이게 데이터포인트 하나로 간주될 것임
...
class mixed_dataset(Dataset):
    ...
    def initialize(self):
        ...
        for data_name, dataset in zip(self.seq_of_data_name, self.seq_of_dataset):
           data += [
                (d, data_name) for d in dataset.dataset
            ]
