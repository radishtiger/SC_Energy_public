...
class set_nli_dataset(Dataset):
    def __init__(self, params, mode = 'train', semantic = 'con', pairwise = 'arbitrary_pairs', load_in_once_everything = False, output_form = 'real_num', input_type = "set"):
        super().__init__()
        self.params = params
        ...
        self.dataset_num = {"train":self.params['set_nli']['stepwise_dataset_train_num'], 
                            "eval":self.params['set_nli']['stepwise_dataset_eval_num'], 
                            "eval2":self.params['set_nli']['stepwise_dataset_eval2_num'], 
                            "test":self.params['set_nli']['stepwise_dataset_test_num']}[self.mode]
    ...
    def load_dataset(self):
        required_raw_dataset = [f"raw_{mode}_file.pickle" for mode in ['train', 'eval', 'eval2', 'test']]
    ...
    def load_dataset_two_pairs_con(self):
        ...
        with open(os.path.join(self.processed_dataset_path, f"raw_{self.mode}_file.pickle"), 'rb') as f:
        ...
        for one_seed_pair in raw_source_file:
            if "pairID" not in one_seed_pair:
                continue
            seed_pair_original_label = one_seed_pair['label'] # one of 'entailment', 'neutral', 'contradiction'
            if seed_pair_original_label == 'entailment':
                sets = one_seed_pair['sets']
                consistent_indices = [i for i in range(len(sets)) if sets[i]['label'] == 'consistent']
    ...
    def load_dataset_two_pairs_incon(self):
        ...
        entailment_two_pair_incon_gen_functions={
            ...
            "p_or_h_and_noth": lambda x: [f"Either {x['premise']} or {x['hypothesis']}.", x['hypothesis_negated']], 
        }
        ...
        contradiction_gen_rules = list(contradiction_two_pair_incon_gen_functions.keys())
        ...
        for one_seed_pair in raw_source_file:
            ...
            if seed_pair_original_label == 'entailment':
                ...
                one_set = entailment_two_pair_incon_gen_functions[generation_rule](one_seed_pair)
            if seed_pair_original_label == 'neutral':
                generation_rule = random.choice(neutral_gen_rules)
                one_set = neutral_two_pair_incon_gen_functions[generation_rule](one_seed_pair)
            if seed_pair_original_label == 'contradiction':
                generation_rule = random.choice(contradiction_gen_rules)
                one_set = contradiction_two_pair_incon_gen_functions[generation_rule](one_seed_pair)
    ...
    def load_dataset_arbitrary_pairs_incon(self):
        ...
        for one_seed_pair in raw_source_file:
            sets = one_seed_pair['sets']
            inconsistent_indices = [i for i in range(len(sets)) if sets[i]['label'] == 'inconsistent']
    ...
    def load_dataset_arbitrary_pairs_con_and_con(self):
        ...
        for i, one_seed_pair in enumerate(raw_source_file):
            ...
            consistent_indices1 = [i for i in range(len(sets1)) if sets1[i]['label'] == 'consistent']
            used_rule_index1 = random.choice(consistent_indices1)
            one_set1 = sets1[used_rule_index1]['set']

            consistent_indices2 = [i for i in range(len(sets2)) if sets2[i]['label'] == 'consistent']
    ...
    def load_dataset_arbitrary_pairs_con_and_incon(self):
        ...
        for i, one_seed_pair in enumerate(raw_source_file):
            ...
            inconsistent_indices2 = [i for i in range(len(sets2)) if sets2[i]['label'] == 'inconsistent']
    ...
    def load_dataset_arbitrary_pairs_incon_and_incon(self):
        ...
        for i, one_seed_pair in enumerate(raw_source_file):
            ...
            inconsistent_indices1 = [i for i in range(len(sets1)) if sets1[i]['label'] == 'inconsistent']
    ...
    def raw_data_split(self):
        ...
        source_json_data = source_json_data['instances'] # type(source_json_data) = list
        ...
        train_file_full, eval_file_full, eval2_file_full, test_file_full = [], [], [], [] # full data

        train_two_pair_seed_index, eval_two_pair_seed_index, eval2_two_pair_seed_index, test_two_pair_seed_index = set(), set(), set(), set()
        ...
        for one_instance in tqdm(source_json_data):
            ...
            elif ("pairID_1" in one_instance) and  ("pairID_2" in one_instance): # Sets are generated by two seed pairs
                if (one_instance["pairID_1"] in train_pairIDs) and (one_instance["pairID_2"] in train_pairIDs):
                    train_file_full.append(one_instance) 
                elif (one_instance["pairID_1"] in eval_pairIDs) and (one_instance["pairID_2"] in eval_pairIDs):
                    eval_file_full.append(one_instance)
                elif (one_instance["pairID_1"] in eval2_pairIDs) and (one_instance["pairID_2"] in eval2_pairIDs):
                    eval2_file_full.append(one_instance)
                elif (one_instance["pairID_1"] in test_pairIDs) and (one_instance["pairID_2"] in test_pairIDs):
                    test_file_full.append(one_instance)

                if (one_instance["pairID_1"], one_instance["pairID_2"]) in train_two_pair_seed_index:
                    train_file.append(one_instance)
                if (one_instance["pairID_1"], one_instance["pairID_2"]) in eval_two_pair_seed_index:
                    eval_file.append(one_instance)
                if (one_instance["pairID_1"], one_instance["pairID_2"]) in eval2_two_pair_seed_index:
                    eval2_file.append(one_instance)
                if (one_instance["pairID_1"], one_instance["pairID_2"]) in test_two_pair_seed_index:
                    test_file.append(one_instance)
        
        for file in (train_file, eval_file, eval2_file, test_file,
                      train_file_full, eval_file_full, eval2_file_full, test_file_full):
            random.shuffle(file)

        # 결과를 저장
        with open(os.path.join(self.processed_dataset_path, 'raw_train_file.pickle'), 'wb') as f:
            pickle.dump(train_file, f)
        with open(os.path.join(self.processed_dataset_path, 'raw_eval_file.pickle'), 'wb') as f:
            pickle.dump(eval_file, f)
        with open(os.path.join(self.processed_dataset_path, 'raw_eval2_file.pickle'), 'wb') as f:
            pickle.dump(eval2_file, f)
        with open(os.path.join(self.processed_dataset_path, 'raw_test_file.pickle'), 'wb') as f:
            pickle.dump(test_file, f)

        with open(os.path.join(self.processed_dataset_path, 'raw_train_file_full.pickle'), 'wb') as f:
            pickle.dump(train_file_full, f)
        with open(os.path.join(self.processed_dataset_path, 'raw_eval_file_full.pickle'), 'wb') as f:
            pickle.dump(eval_file_full, f)
        with open(os.path.join(self.processed_dataset_path, 'raw_eval2_file_full.pickle'), 'wb') as f:
            pickle.dump(eval2_file_full, f)
        with open(os.path.join(self.processed_dataset_path, 'raw_test_file_full.pickle'), 'wb') as f:
            pickle.dump(test_file_full, f)

        print("raw data split ends.")
        print(f"raw_train_filename: {os.path.join(self.processed_dataset_path, 'raw_train_file.pickle')}")
        print(f"raw_train dataset length: {len(train_file)}")
        print(f"raw_eval_filename: {os.path.join(self.processed_dataset_path, 'raw_eval_file.pickle')}")
        print(f"raw_eval dataset length: {len(eval_file)}")
        print(f"raw_eval2_filename: {os.path.join(self.processed_dataset_path, 'raw_eval2_file.pickle')}")
        print(f"raw_eval2 dataset length: {len(eval2_file)}")
        print(f"raw_test_filename: {os.path.join(self.processed_dataset_path, 'raw_test_file.pickle')}")
