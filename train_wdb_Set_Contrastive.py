...
from trainer.modules import train_energy, evaluate_energy_fine_grained, locate_energy, learn_energy_threshold
from trainer.modules import train_supervised, evaluate_supervised_one_semantic, locate_energy, learn_energy_threshold, evaluate_energy_one_semantic, evaluate_energy_one_to_one, evaluate_supervised_one_to_one

from tasks.dataset_loader import dataset_loader, dataset_loader_fine_grained, concat_arbitrary_pairs
...
sweep_configuration = {
    ...
    'parameters': 
    {
        ...
        # lossNet_lr: range(5,8)에 포함되는 경우 roberta-base 잘 학습 됨을 확인.
        
        'triplet_margin': {'values': [pow(10, -1*i) for i in range(2, 3)]}, 
        # triplet_margin: range(1,3)에 포함되는 경우 roberta-base 잘 됨을 확인.
...
sw_count = math.prod([len(val['values']) for val in sweep_configuration['parameters'].values()])
sw_count = 1

def main():

    folder_path = os.path.join('results', params['task'], params['dataset'], str(params['job_id']))
    ...
    # Load dataset
    train_con_dataset_load_all = dataset_loader_fine_grained(params, 'train', 'con', load_in_once_everything = True)
    train_incon_dataset_load_all = dataset_loader_fine_grained(params, 'train', 'incon', load_in_once_everything = True)
    ...
    eval2_con_dataset_arbitrary_pairs = dataset_loader_fine_grained(params, 'eval2', 'con', pairwise='arbitrary_pairs')
    eval2_incon_dataset_arbitrary_pairs = dataset_loader_fine_grained(params, 'eval2', 'incon', pairwise='arbitrary_pairs')
    ...
    if params['energynet']['decomposition'] == 'no':
        train_con_dataloader_load_all = no_decomposition_loader(train_con_dataset_load_all, params = params, tokenizer = lossNet.representation_model.tokenizer).get_loader()
        train_incon_dataloader_load_all = no_decomposition_loader(train_incon_dataset_load_all, params = params, tokenizer = lossNet.representation_model.tokenizer).get_loader()
    ...
    for e_total in range(params['energynet']['epoch']):
        ...
        train_loss = 1000
        for ts_idx, (pos_train_dataloader, neg_train_dataloader) in enumerate(training_steps):
            ...
            if ts_idx == 0 or ts_idx == 4:
                ...
                lt = False
            train_loss_step, threshold_result = train_energy(lossNet, energy_opt, pos_train_dataloader, neg_train_dataloader, lt, device, params)
        ...
        for es_idx, eval_dataloader in enumerate(eval2_steps):
            ...
            f"eval_loss-{es_idx+1}-{eval2_steps_names[es_idx]}": eval_result.get('eval_loss', 0),
            f"eval_auroc-{es_idx+1}-{eval2_steps_names[es_idx]}": eval_result.get('auroc', 0),
            f"eval_precision-{es_idx+1}-{eval2_steps_names[es_idx]}": eval_result.get('eval_precision', 0),
            f"eval_recall-{es_idx+1}-{eval2_steps_names[es_idx]}": eval_result.get('eval_recall', 0),
            f"eval_f1-{es_idx+1}-{eval2_steps_names[es_idx]}": eval_result.get('eval_f1', 0),
            f"eval_acc-{es_idx+1}-{eval2_steps_names[es_idx]}": eval_result.get('eval_acc', 0),
            f"eval_oracle_acc-{es_idx+1}-{eval2_steps_names[es_idx]}": eval_result.get('eval_oracle_acc', 0),
        ...
        if best_eval_total_acc < result['eval_total_acc']:
            ...
            torch.save({
                    ...
                    "threshold": lossNet.threshold
                    }, params['model_path'])
            print("model saved.")
        print(f"best_eval_total_acc: {best_eval_total_acc:.4f}\ncurrent eval total acc: {result['eval_total_acc']:.4f}")
